...
from exceptions import NoInternet, UnableToTranslate, UnknownError
...
class Async_PyTranslator:
    ...
    async def detect_lang(self, text):
        ...
        async with ClientSession() as client:
            async with client.post("https://www.translate.com/translator/ajax_lang_auto_detect", data={"text_to_translate": str(text)}) as fetch:
    ...
    async def google(self, text, dest):
        ...
        try:
            async with ClientSession() as client:
                async with client.get(f"https://clients5.google.com/translate_a/t?client=at&sl=auto&tl={dest}&q={text}") as fetch:
